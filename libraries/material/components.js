...
prx.commonproperties.materialRipple = {
	...
	properties: [[
		{
			caption: 'Ripple effect on tap'
			, name: 'rippleEffect'
			, type: 'onoff'
			, value: function (item, name) {
			return item.rippleEffect;
		}
			, onChange: function (item, name) {
			$('#property-rippleEffectColor').toggle(item.rippleEffect);
		}
			, changeProperty: {
			caption: 'Ripple effect',
			rerender: true
...
prx.types.material_tabbar_fixed = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {	
		...
		if(!prx.editor) {
		cR += '#' + _id + ' .material-tabbar-fixed-indicator { width: '+width+'px; height: '+item.tabIndicatorWidth+'px; background-color: '+prx.utils.getColor(item.tabIndicatorColor)+ '; }';
		}
		else {
			cR += '#' + _id + ' .material-tabbar-fixed-list-item-input + label { border-bottom: '+item.tabIndicatorWidth+'px solid transparent; }';
			cR += '#' + _id + ' .material-tabbar-fixed-list-item-input:checked + label { border-bottom: '+item.tabIndicatorWidth+'px solid '+prx.utils.getColor(item.tabIndicatorColor)+ ';}';
		}
		cR += '</style>';		
		...
		$.each(item.tabs, function(i,elm){			
			...
			cR += '<li class="material-tabbar-fixed-list-item liveUpdate-backgroundColor liveUpdate-textColor liveUpdate-textColorActive changeProperty-backgroundColor changeProperty-textColor changeProperty-textColorActive changeProperty-textSize changeProperty-textFont dynamic-property '+((item.rippleEffect) ? 'prx-material-ripple' : '' )+'" data-dynamic-property-index="'+i+'" id="'+_id+'-tabs-'+i+'" '+((elm.linkedscreen != -1) ? 'data-linked-screen="'+elm.linkedscreen+'"' : '')+'>';
			...
			cR += '<label class="material-tabbar-fixed-list-item-label" for="'+_id+'-radio-'+i+'"><span data-editableproperty="caption" data-dynamic-property-index="'+i+'">'+elm.caption+'</span></label>';
			cR += '</li>'; 			
		});
		
		cR += '</ul>';
		cR += '<div class="material-tabbar-fixed-indicator"></div>'
		cR += '</div>';		
		return cR;
	}
...
prx.types.material_tabbar_scrollable = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '#' + _id + ' .material-tabbar-scrollable-list-item { background-color: '+prx.utils.getColor(item.backgroundColor)+'; font-size: '+item.textSize+'px; color: '+prx.utils.getColor(item.textColor)+'; '+prx.componentsHelper.getFontCssFromFontFamily(item.textFont)+' '+_props+' }';
		cR += '#' + _id + ' .material-tabbar-scrollable-list-item-input + label { border-bottom: '+item.tabIndicatorWidth+'px solid transparent; }';
		if(prx.editor) {
			cR += '#' + _id + ' .material-tabbar-scrollable-list-item-input:checked + label { border-bottom: '+item.tabIndicatorWidth+'px solid '+prx.utils.getColor(item.tabIndicatorColor)+ '; color: '+prx.utils.getColor(item.textColorActive)+';}';
		} else {
		...
		$.each(tabs, function(i,elm){
			...
			cR += '<li class="material-tabbar-scrollable-list-item liveUpdate-backgroundColor liveUpdate-textColor liveUpdate-textColorActive changeProperty-backgroundColor changeProperty-textColor changeProperty-textColorActive changeProperty-textSize changeProperty-textFont dynamic-property '+((item.rippleEffect) ? 'prx-material-ripple' : '' )+'" data-dynamic-property-index="'+i+'" id="'+_id+'-tabs-'+i+'">';
			...
			cR += '<label class="material-tabbar-scrollable-list-item-label" for="'+_id+'-radio-'+i+'"><span data-editableproperty="caption" data-dynamic-property-index="'+i+'">'+elm+'</span></label>';
			cR += '</li>'; 			
		});
		
		cR += '</ul>';
		cR += '<div class="material-tabbar-scrollable-indicator"></div>'
		cR += '</div>';
		cR += '</div>';
		return cR;
	}
...
prx.types.material_morphing_lines = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>'
		if(prx.editor) { 
			cR += '#' + _id + ' label, #' + _id + ' .lines, #' + _id + ' .lines:before, #' + _id + ' .lines:after {  transition: none; }';
		}
		cR += '#' + _id + ' .lines { width: '+Math.round(dims.width*0.33)+'px; height: '+Math.round(dims.height/22)+'px; }';
		cR += '#' + _id + ' .lines, #' + _id + ' .lines:before, #' + _id + ' .lines:after { background-color: '+prx.utils.getColor(item.backgroundColor)+'; }';
		cR += '#' + _id + ' input:checked + label .lines, #' + _id + ' input:checked + label .lines:before, #' + _id + ' input:checked + label .lines:after { background-color: '+prx.utils.getColor(item.backgroundColorActive)+'; }';
		cR += '</style>'
		...
		cR += '<span class="lines liveUpdate-backgroundColor liveUpdate-backgroundColorActive changeProperty-backgroundColor changeProperty-backgroundColorActive"></span>';
		cR += '</label>';
		cR += '</div>';
		return cR;
	}
...
prx.types.material_textfield = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		if(prx.editor) {
			...
			cR += '<div class="faux-input liveUpdate-backgroundColor liveUpdate-textColor liveUpdate-borderColor" data-editableproperty="value">'+item.value+'</div>';
			cR += '<div class="faux-input placeholder-input liveUpdate-backgroundColor liveUpdate-textColor liveUpdate-borderColor">'+item.placeholder+'</div>'
			cR += '</div>';
		}
		else {
			if(item.hasIcon && item.imgSrc.url != '') {
...
prx.types.material_textarea = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		var cR = '';		
		cR += '<div id="' + _id + '" class="box pos type-material-textarea'+((item.isFull) ? ' full' : '')+'">';
...
prx.types.material_textfield_floating_label = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		var cR = '';
		cR += '<div id="' + _id + '" class="box pos type-material-floating-label type-material-textfield-'+item.inputtype+'">';
		
		cR += '<style>';
		cR += '#'+_id+' input, #'+_id+' textarea, #'+_id+' .faux-input { background-color: '+prx.utils.getColor(item.backgroundColor)+'; color: '+prx.utils.getColor(item.textColor)+'; font-size: '+item.textSize+'px; '+prx.componentsHelper.getFontCssFromFontFamily(item.textFont)+' border-bottom: '+item.borderWidth+'px solid '+prx.utils.getColor(item.borderColor)+'; text-align: '+item.textAlign+'; '+ _props + ' }';
		...
		if(prx.editor) {
			...
			cR += '<input type="'+item.inputtype+'" value="'+item.value+'" data-role="none" class="real-input changeProperty-backgroundColor changeProperty-textColor changeProperty-textSize changeProperty-textFont changeProperty-borderColor changeProperty-borderWidth changeProperty-borderRadius changeProperty-textAlign" />'
			}
			else if(item.fieldtype == 'textarea') {
			cR += '<textarea data-role="none" class="real-input liveUpdate-backgroundColor liveUpdate-textColor liveUpdate-borderColor changeProperty-backgroundColor changeProperty-borderColor changeProperty-borderWidth changeProperty-borderRadius changeProperty-textSize changeProperty-textFont changeProperty-textColor changeProperty-textAlign">'+item.value.replace(/<br \/>/g, "\n")+'</textarea>'
			}
			cR += '<div class="label '+((item.fieldtype == 'textarea') ? 'textarea ' : '')+'liveUpdate-labelColor changeProperty-labelAlign">'+item.label+'</div>'
			cR += '</div>';
		}
...
prx.types.material_ripple = {
	...
	,onDisplay: function(item,containerid) {
		...
		cR += '<div id="' + _id + '" class="box pos type-material-ripple"><div class="inner-rec liveUpdate-rippleEffectColor" ><div></div></div></div>';
		return cR;
	}
	,interactions: [
		prx.commonproperties.actions
	...
	,afterDisplay: function(item,containerid,pageid,symbol) {
		...
		if(!prx.editor) {
			...
			prx.draggable._draggables['#'+_id+' .material-slider-button-wrapper'] = Draggable.create('#'+_id+' .material-slider-button-wrapper', {
				type: 'x',
				bounds: { left: $('#'+_id).offset().left - (dims.height/4), width: parseInt(dims.width)+parseInt(dims.height/4)+(dims.height/4), top: 0, height: 0 },
